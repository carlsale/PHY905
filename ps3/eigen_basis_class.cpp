//  file: eigen_basis_class.cpp
// 
//  Program to find bound state eigenvalues for various potentials
//   by diagonalizing the Hamiltonian in a truncated harmonic
//   oscillator basis using a Hamiltonian class that
//  is a "wrapper" for the GSL eigenvalue/eigenvector routines. 
//
//  Programmer:  Dick Furnstahl  furnstahl.1@osu.edu
//
//  Revision history:
//      01/28/09  modified eigen_basis.cpp to move the GSL
//                 dependent parts to a Hamiltonian class.
//	27-Feb-2019  added output of total radial wavefunctions for range of r
//                   to file at different b_ho and dimensions, added a check
//                   for normalization of each wavefunction (Programmer: Alex Carls)
//
//  Notes:
//   * Had to re-index from 1 instead of from 0
...
/*
     ...
     Analysis of plots in eigen_basis_b_0.5and1.2.ps and eigen_basis_b_2and6.ps:
	The one value for b_ho which had the least deviation between the trials of
	differing dimensions is for b_ho = 1.2. For all the other cases, the ground
	state wavefunction looked very different at each dimension size. Additionally,
	the wavefunctions for this value of b_ho were very close to being normalized.
...
main ()
{
  ...
  my_filename_stream << "eig_b" << setprecision(3) << b_ho << "_N_" << dimension << ".out";
  ...
  for (int i = 1; i <= dimension; i++)
    {
      ...
      my_file <<  scientific << setprecision(9) << eigenvalue << " ";
    }
  ...
  double normalization[dimension];
  ...
  for (double r = 0.1; r <= rMax; r += dr)
    {
      my_file << endl << setw(14) << r;
      my_file << " ";
      for (int i = 1; i <= dimension; i++)
        {
          wavefunction = 0.;
          for (int j = 1; j <= dimension; j++)
          {
            eigvec = my_hamiltonian.get_eigenvector(i,j);
            ho_wf = ho_radial(i,j,b_ho,r);
            wavefunction += eigvec * ho_wf;
          }
          my_file <<  scientific << setprecision(8) << wavefunction << " ";
          normalization[i] += wavefunction * wavefunction *dr;
        }
    }

  cout << endl << "Checking Normalization of each wavefunction..." << endl;
  for (int i = 1; i <= dimension; i++)
    {
      cout << "   int^{rMax}_{0} u_{" << i << "}(r) dr = " << normalization[i] << endl;
    }
