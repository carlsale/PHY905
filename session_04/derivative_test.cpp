...
main (void)
{
  ...
  func_params my_parameters;

  my_parameters.alpha = 2.;
  my_parameters.beta = 3./2.;

  ofstream out ("derivative_test.dat");	// open the output file 

  params_ptr = &my_parameters;		// double to pass to function 
  ...
  out << "#log10(h)  FD Error   CD Error   Extrapolation Error" << endl;
...
{
  func_params *passed_ptr;
  passed_ptr = (func_params *) params_ptr;

  double passed_alpha = passed_ptr->alpha;
  double passed_beta = passed_ptr->beta;

  return (passed_alpha * exp (-passed_beta * x));
}
...
test_function_derivative (double x, void *params_ptr)
{
  ...
  return (-passed_alpha * passed_beta * exp (-passed_beta * x));
}
