...
main (void)
{
  ...
  while (answer2 != 0)    // iterate until told to move on 
  {
    ...
    filename << "diffeq_pendulum_alpha" << setprecision(2) << alpha << ".dat";

    ofstream out;    // declare the output file
    out.open (filename.str(), ofstream::trunc); 
    ...
    for (double t = tmin; t <= tmax; t += h)
    {
      ...
      if ((t >= plot_min) & (t <= plot_max))
      {
        ...
        {    // plot every plot_skip points 
          ...
          myPipe.plot (theta, theta_dot);  // uncomment for phase space plot
          //myPipe.plot (t+h, theta_dot);  // uncomment for theta vs time plot
        }
        if ((point_count % T_skip) == 0)
        {    // plot every T_skip points 
             // send points to gnuplot
          myPipe.plot2 (theta, theta_dot);  // uncomment for phase space plot
          //myPipe.plot2 (t+h, theta);  // uncomment for theta vs time plot
        }
      }
    }  // end for loop over t
