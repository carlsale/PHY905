...
# fit the curve
f1(x) = a1*x + b1
fit [-10:-8] f1(x) "derivative_test.dat" using ($1):($2) via a1,b1 
fit_title1 = sprintf("%-+4.1f*x %-+4.1f",a1,b1)

f2(x) = a2*x + b2
fit [-10:-5] f2(x) "derivative_test.dat" using ($1):($3) via a2,b2
fit_title2 = sprintf("%-+4.1f*x %-+4.1f",a2,b2)

f3(x) = a3*x + b3
fit [-10:-2.5] f3(x) "derivative_test.dat" using ($1):($4) via a3,b3
