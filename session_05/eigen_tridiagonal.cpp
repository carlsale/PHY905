...
double eigenvalue, error;
double actual = 1.5;

      ofstream eigout ("eigen_tridiagonal_error2.dat");  // open an output file
      eigout << "# 3D harmonic oscillator" << endl;
      eigout << "# eigenvalue = " << scientific << actual << endl;
      eigout << endl << "#log10(N)              log10(error)" << endl;
...
*/
  ...
  eigenvalue = gsl_vector_get (Eigval_ptr, 0);
  error = log10(fabs ((eigenvalue - actual) / actual));
        eigout << fixed << scientific << setprecision(16) << log10(k) << " "
	     << error << endl;
