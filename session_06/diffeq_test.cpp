...
main (void)
{
  ...
  ofstream out ("test.dat");	// open the output file 
  ...
  for(double h = 0.5; h >= 0.000001; h /= 1.2)
  {
    ...
    double exact = exact_answer (t + h, params_ptr);

    out << scientific << setprecision (16)
      << log10(h) << "  "
      << log10(fabs(y_euler[0]-exact)/exact) << "  "
      << log10(fabs(y_rk4[0]-exact)/exact) << endl;
  }
