...
u	      break;
	    ...
	    case 12:
	      ...
	      break;
	    case 13:
	      cout << " enter damping parameter, b: ";
	      cin >> b;
	      break;
	    default:
	      break;
      ...
      else  // append
	{
      ...
      out << fixed << setprecision(4) << tmin << "  "
          ...
          << scientific << setprecision(15) <<  
	       potential (x0, rhs_params_ptr) << "  " 
          << scientific << setprecision(15) <<  
	       (potential (x0, rhs_params_ptr) + m*v0*v0/2.) << endl;
      ...
      for (t = tmin; t <= tmax; t += h)
	{
	  ...
	  keSum += m*v*v/2.;
	  peSum += potential (x, rhs_params_ptr);
	  ...
	  if ((point_count % plot_skip) == 0)
	    {			// plot every plot_skip points 
              ...
              out << fixed << setprecision(4) << t+h << "  "
                  ...
                  << scientific << setprecision(15) <<  
		       potential (x, rhs_params_ptr) << "  " 
          << scientific << setprecision(15) <<  
	       (keSum-peSum)/point_count << endl;
	    }
	}
...
rhs (double t, double y[], int i, void *params_ptr)
{
  ...
  double F_ext = f_ext * cos (omega_ext * t + phi_ext) - b * y[1];
