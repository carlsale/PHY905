...
main (void)
{
  ...
  // square two numbers by calling a function that multiplyies them together 
  gettimeofday (&start, NULL);     // start the clock to time the next section 
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  // square two numbers by calling a macro 
  gettimeofday (&start, NULL);     // start the clock to time the next section 
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
  ...
  cout << "final y = " << y << endl;  // prevent optimizing away loop
  gettimeofday (&stop, NULL);      // stop the clock and print elapsed time 
  cout << " Evaluating " << repeat << " sqr(x) macro's took "
    << elapsed (start, stop) << " seconds" << endl << endl;

  // square two numbers by using an inline function
  gettimeofday (&start, NULL);     // start the clock to time the next section 
  for (int i = 0; i < repeat; i++)
  {
    x = double(i);
