...
double eigenvalue_gs = -45.93207285858243;

ofstream file;
file.open("eigen_basis_4b.dat");
file << "#Dimension         error" << endl;

for(int index = 1; index <= 48; index++)
{
//  b_ho = index*b_interval;
...
double error = fabs(eigenvalue - eigenvalue_gs)/fabs(eigenvalue_gs);
file << setprecision(16) << log10(dimension) << " " << log10(error) << endl;
}
